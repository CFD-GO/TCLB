...
struct ParticleI : Particle {
	...
	CudaDeviceFunction ParticleI(const size_t& i_, const real_t node[3]): i(i_) {
		vector_t pos, vel, angvel;
		rad = constData.particle_data[i*RFI_DATA_SIZE+RFI_DATA_R];
		pos.x = constData.particle_data[i*RFI_DATA_SIZE+RFI_DATA_POS+0];
		pos.y = constData.particle_data[i*RFI_DATA_SIZE+RFI_DATA_POS+1];
		pos.z = constData.particle_data[i*RFI_DATA_SIZE+RFI_DATA_POS+2];
		vel.x = constData.particle_data[i*RFI_DATA_SIZE+RFI_DATA_VEL+0];
		vel.y = constData.particle_data[i*RFI_DATA_SIZE+RFI_DATA_VEL+1];
		vel.z = constData.particle_data[i*RFI_DATA_SIZE+RFI_DATA_VEL+2];
		angvel.x = constData.particle_data[i*RFI_DATA_SIZE+RFI_DATA_ANGVEL+0];
		angvel.y = constData.particle_data[i*RFI_DATA_SIZE+RFI_DATA_ANGVEL+1];
		angvel.z = constData.particle_data[i*RFI_DATA_SIZE+RFI_DATA_ANGVEL+2];
		diff.x = pos.x - node[0];
		diff.y = pos.y - node[1];
		diff.z = pos.z - node[2];
...
CudaDeviceFunction ParticleS< NO_SYNC >::~ParticleS() {
	CudaAtomicAdd(&constData.particle_data[i*RFI_DATA_SIZE+RFI_DATA_FORCE+0],force.x);
	CudaAtomicAdd(&constData.particle_data[i*RFI_DATA_SIZE+RFI_DATA_FORCE+1],force.y);
	CudaAtomicAdd(&constData.particle_data[i*RFI_DATA_SIZE+RFI_DATA_FORCE+2],force.z);
	CudaAtomicAdd(&constData.particle_data[i*RFI_DATA_SIZE+RFI_DATA_MOMENT+0],moment.x);
	CudaAtomicAdd(&constData.particle_data[i*RFI_DATA_SIZE+RFI_DATA_MOMENT+1],moment.y);
	CudaAtomicAdd(&constData.particle_data[i*RFI_DATA_SIZE+RFI_DATA_MOMENT+2],moment.z);
}

template <>
CudaDeviceFunction ParticleS< WARP_SYNC >::~ParticleS() {
	real_t val[6] = {force.x,force.y,force.z,moment.x,moment.y,moment.z};
	CudaAtomicAddReduceWarpArr<6>(&constData.particle_data[i*RFI_DATA_SIZE+RFI_DATA_FORCE],val);
}
...
CudaDeviceFunction ParticleS< OPP_SYNC >::~ParticleS() {
	real_t val[6] = {force.x,force.y,force.z,moment.x,moment.y,moment.z};
	CudaAtomicAddReduceOppArr<6>(&constData.particle_data[i*RFI_DATA_SIZE+RFI_DATA_FORCE],val);
}
#endif
...
CudaDeviceFunction ParticleS< BLOCK_SYNC >::~ParticleS() {
	CudaAtomicAddReduce(&constData.particle_data[i*RFI_DATA_SIZE+RFI_DATA_FORCE+0],force.x);
	CudaAtomicAddReduce(&constData.particle_data[i*RFI_DATA_SIZE+RFI_DATA_FORCE+1],force.y);
	CudaAtomicAddReduce(&constData.particle_data[i*RFI_DATA_SIZE+RFI_DATA_FORCE+2],force.z);
	CudaAtomicAddReduce(&constData.particle_data[i*RFI_DATA_SIZE+RFI_DATA_MOMENT+0],moment.x);
	CudaAtomicAddReduce(&constData.particle_data[i*RFI_DATA_SIZE+RFI_DATA_MOMENT+1],moment.y);
	CudaAtomicAddReduce(&constData.particle_data[i*RFI_DATA_SIZE+RFI_DATA_MOMENT+2],moment.z);
}
...
CudaDeviceFunction T ParticleIteratorXBlockT(real_t x, real_t y, real_t z) {
	...
	return T(constData.solidfinder, point, lower, upper);
}
