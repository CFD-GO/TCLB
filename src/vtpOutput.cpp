...
void vtpFileOut::Init(lbRegion regiontot, lbRegion region, size_t latticeSize, char* selection, double spacing) {
	...
	fprintf(f, "<PolyData>\n");
	fprintf(f, "<Piece NumberOfPoints=\"%lu\" NumberOfVerts=\"0\" NumberOfLines=\"0\" NumberOfStrips=\"0\" NumberOfPolys=\"0\">\n", latticeSize);
	...
	for (int i=0;i<size;i++)
	{
		...
		if (fp != NULL) {
			fprintf(fp, "<Piece NumberOfPoints=\"%lu\" NumberOfVerts=\"0\" NumberOfLines=\"0\" NumberOfStrips=\"0\" NumberOfPolys=\"0\" Source=\"%s\"/>\n", locSize, buf);
		}
	}
	delete[] buf;
	if (fp != NULL) {
