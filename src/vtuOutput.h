...
class vtuFileOut {
    ...
    MPI_Comm comm;
public:
    vtuFileOut(MPI_Comm comm_ = MPI_COMM_WORLD);
    int Open(const char* filename);
	void WriteB64(void * tab, int len);
	void Init(lbRegion region, size_t nPoints, size_t nCells, char* selection);
	void Init(lbRegion, lbRegion region, size_t nPoints, size_t nCells, char* selection, double spacing);
	inline void Init(lbRegion tot, lbRegion region, size_t nPoints, size_t nCells, char* selection) { Init(tot, region, nPoints, nCells, selection, 0.05); }
	void Init(int width, int height, size_t nPoints, size_t nCells);
	void WriteField(const char * name, void * data, int elem, const char * tp, int components);
	void WriteField(const char * name, void * data, int elem, int dataLength, const char * tp, int components);
	...
	void WritePointsFooter();
	void WriteCellsHeader();
	void WriteCellsFooter();
	void Finish();
	void Close();
};

#endif
#define VTUOUTPUT_H 1