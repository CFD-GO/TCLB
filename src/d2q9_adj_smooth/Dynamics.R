...
Quantities = data.frame(
        name = c("Rho","U","RhoB","UB","W","WB","W0"),
        type = c("real_t","vector_t","real_t","vector_t","real_t","real_t","real_t"),
	adjoint = c(F,F,T,T,F,T,F)
)
...
Settings = table_from_text("
	...
	Smooth			  NA			  NA   'smoothing parameter'
	Penalty			  NA			  NA   'penalty for topopt'
")
...
Density = rbind(Density, data.frame(
	name = c("w","w0","w1","w2","w3","w4"),
	dx=c(0,0,1,0,-1,0),dy=c(0,0,0,1,0,-1),dz=0,
	group="w",
	comment = "Porocity"
))
