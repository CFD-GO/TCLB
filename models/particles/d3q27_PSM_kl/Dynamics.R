...
AddSetting(name="tau", default=1.0, comment='relaxation time')
AddSetting(name="nu", tau='(3*nu+0.5)', default=0.1, comment='kinetic viscosity in LBM unit')
AddDensity(name="omegaF", group="Viscosity", comment='one over F relaxation time')
...
if (Options$particles) {
	if (Options$singlekernel) {
            AddStage("BaseInit", "Init", save = Fields$group %in% c("f","Force","Viscosity"), load = FALSE)
            AddStage("BaseIteration", "Run", save = Fields$group %in% c("f","Force","Viscosity"), load = DensityAll$group %in% c("f","Viscosity"))
            AddAction("Iteration", "BaseIteration")
            AddAction("Init", "BaseInit")
	} else {
            AddStage("BaseInit", "Init", save = Fields$group %in% c("f","Viscosity"), load = FALSE)
            AddStage("BaseIteration", "Run", save = Fields$group %in% c("f","Viscosity"), load = DensityAll$group %in% c("f","Force","Viscosity"))
            AddStage("CalcF", save=Fields$group %in% "Force", load = DensityAll$group %in% c("f","Viscosity"), particle=TRUE)
            AddAction("Iteration", c("BaseIteration", "CalcF"))
            AddAction("Init", c("BaseInit", "CalcF"))
	}
} else {
        AddStage("BaseInit", "Init", save = Fields$group %in% c("f","Viscosity"), load = FALSE)
        AddStage("BaseIteration", "Run", save = Fields$group %in% c("f","Viscosity"), load = DensityAll$group %in% c("f","Viscosity"))
        AddAction("Iteration", "BaseIteration")
        AddAction("Init", "BaseInit")
}
