...
def createConfig( R, H, LdoR, hdoR, rhoW, prefix ):
    ...
    CLBc.addSolve(iterations=500 * NX, vtk=100 * NX) 
    
    #CLBc.dump()
    #f = file('/tmp/list.txt', 'a')
    print fname+'_VTK_P00_%.8d.vti'%int(500*NX+1)
    import os	
  ...
  for rhoW in [0.5, 1.5]:
      ...
      for LdoR in range(1,30):
          ...
          for V in [0.005]:
              hdoR = V / LdoR
       #       print hdoR
              createConfig(R,H,LdoR,hdoR,rhoW, '/home/uqtmitc3/TCLB/example/tmp/matrix3.2-')
